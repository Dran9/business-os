...
router.get('/dashboard', authMiddleware, tenantMiddleware, async (req, res) => {
  try {
    ...
    const [[leadsTotal], [leadsNew], [leadsConverted], [workshopsActive], [income], [expense]] = await Promise.all([
      query('SELECT COUNT(*) as c FROM leads WHERE tenant_id = ? AND created_at >= ?', [tid, monthStart]),
      query('SELECT COUNT(*) as c FROM leads WHERE tenant_id = ? AND status = "new" AND created_at >= ?', [tid, monthStart]),
      query('SELECT COUNT(*) as c FROM leads WHERE tenant_id = ? AND status = "converted" AND converted_at >= ?', [tid, monthStart]),
...
router.get('/funnel', authMiddleware, tenantMiddleware, async (req, res) => {
  try {
    ...
    const [statuses, [withConversation], [enrolled], [paid], [referidos]] = await Promise.all([
      query('SELECT status, COUNT(*) AS c FROM leads WHERE tenant_id = ? GROUP BY status', [tid]),
      query('SELECT COUNT(DISTINCT lead_id) AS c FROM conversations WHERE tenant_id = ?', [tid]),
      query('SELECT COUNT(*) AS c FROM enrollments WHERE tenant_id = ?', [tid]),
      query("SELECT COUNT(*) AS c FROM enrollments WHERE tenant_id = ? AND payment_status = 'paid'", [tid]),
      query('SELECT COUNT(*) AS c FROM leads WHERE tenant_id = ? AND referred_by_lead_id IS NOT NULL', [tid]),
    ]);
    ...
    const [newLeads, convertedLeads] = await Promise.all([
      query(
        `SELECT DATE_FORMAT(DATE(DATE_SUB(created_at, INTERVAL WEEKDAY(created_at) DAY)), '%Y-%m-%d') AS week_start,
                COUNT(*) AS total
         FROM leads WHERE tenant_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL 8 WEEK)
      ...
      query(
        `SELECT DATE_FORMAT(DATE(DATE_SUB(converted_at, INTERVAL WEEKDAY(converted_at) DAY)), '%Y-%m-%d') AS week_start,
                COUNT(*) AS total
         FROM leads WHERE tenant_id = ? AND status = 'converted' AND converted_at >= DATE_SUB(NOW(), INTERVAL 8 WEEK)
         GROUP BY week_start ORDER BY week_start ASC`,
        [tid],
      ),
    ]);
  ...
  try {
    ...
    const rows = await query(
      `SELECT w.id, w.name, DATE_FORMAT(w.date, '%Y-%m-%d') AS date, w.max_participants, w.price,
              ...
              COALESCE((SELECT SUM(amount) FROM transactions t WHERE t.workshop_id=w.id AND t.tenant_id=w.tenant_id AND t.type='income'), 0) AS revenue
       FROM workshops w
...
router.get('/flow-dropoff', authMiddleware, tenantMiddleware, async (req, res) => {
  try {
    ...
    const [rows, [totalAbandoned]] = await Promise.all([
      ...
      query("SELECT COUNT(*) AS c FROM flow_sessions WHERE tenant_id=? AND status='abandoned'", [tid]),
    ]);
...
router.get('/kpis', authMiddleware, tenantMiddleware, async (req, res) => {
  try {
    ...
    const [[income], [pending], [timeToPay], [referidos]] = await Promise.all([
      query("SELECT COALESCE(SUM(amount),0) AS total FROM transactions WHERE tenant_id=? AND type='income'", [tid]),
      query(
        `SELECT COALESCE(SUM(w.price),0) AS total
         FROM enrollments e
         JOIN workshops w ON w.id=e.workshop_id
         WHERE e.tenant_id=? AND e.payment_status NOT IN ('paid')`,
        [tid],
      ),
      query(
        `SELECT ROUND(AVG(DATEDIFF(e.enrolled_at, l.created_at)),1) AS avg_days
         ...
         WHERE e.tenant_id=? AND e.payment_status='paid' AND l.created_at IS NOT NULL`,
        [tid],
      ),
      query('SELECT COUNT(*) AS c FROM leads WHERE tenant_id=? AND referred_by_lead_id IS NOT NULL', [tid]),
    ]);
    ...
    const [cities, scores, topTags, topLtv] = await Promise.all([
      query(
        `SELECT COALESCE(NULLIF(TRIM(city),''),'Sin ciudad') AS city, COUNT(*) AS total
         FROM leads WHERE tenant_id=? GROUP BY city ORDER BY total DESC LIMIT 8`,
        [tid],
      ),
      query(
        `SELECT
         ...
         FROM leads WHERE tenant_id=? AND quality_score IS NOT NULL`,
        [tid],
      ),
      query(
        `SELECT category, value, COUNT(*) AS total
         FROM tags WHERE tenant_id=? AND target_type='lead'
  ...
  try {
    ...
    const rows = await query(
      `SELECT DATE_FORMAT(date, '%Y-%m') AS month, COALESCE(SUM(amount),0) AS total
       FROM transactions
       WHERE tenant_id=? AND type='income' AND date >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
